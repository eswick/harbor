...
@implementation HBDownloadController
...
- (void)error:(NSString*)text{
	...
	if(self.shouldUseFallbackServer){
		UIAlertView *alert = [[UIAlertView alloc] initWithTitle:@"Error Occurred" message:[NSString stringWithFormat:@"An error occurred while downloading the license for %s: %@", PRODUCT_NAME, text] delegate:self cancelButtonTitle:@"OK" otherButtonTitles:@"Try Again", nil];
		alert.tag = TAG_ERROR;
		[alert show];
		[alert release];
	}else{
...
- (void)connection:(NSURLConnection*)connection didReceiveData:(NSData*)data{
    ...
    self.alertView.title = [NSString stringWithFormat:@"Downloading License\nPlease Wait... (%.0f%%)", self.progress * 100];
}
